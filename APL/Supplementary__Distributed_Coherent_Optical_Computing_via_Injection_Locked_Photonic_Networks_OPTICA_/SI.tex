...
\textcolor{red}{Classification is specific to the analysis window: small residual drift can pass a finite-window threshold. The maps therefore describe approximately steady operation under the stated criteria, without establishing threshold-independent stability boundaries.}
\textcolor{red}{For the unmodulated main-text locking maps, the grid spans $-60$ to $0$~dB in 1~dB steps and $-30$ to $30$~GHz in 25~MHz steps. Each point starts independently from $E(0)=0$ and $N(0)=N_{\mathrm{th}}=N_0+1/(g_0\tau_s)$, with $R_p=1.15N_{\mathrm{th}}/\tau_n$ and $\kappa_{\mathrm{scale}}=1$. Fourth-order Runge--Kutta integration uses a 0.2~ps step for 90,000 steps (18~ns); only the last 3,000 samples (0.6~ns) enter the intensity and unwrapped-phase metrics. The constant injected field is $E_{\mathrm{inj}}=A_{\mathrm{fr}}10^{R_{\mathrm{inj,dB}}/20}$ with zero phase, where $A_{\mathrm{fr}}$ is the free-running steady-state field amplitude from the same model. No stochastic noise is included.}
